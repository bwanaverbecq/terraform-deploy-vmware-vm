...
resource "vsphere_virtual_machine" "laboratory" {
  count            = length(var.ips)
  name             = "${var.guest_name_prefix}0${count.index + 1}"
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = "${var.guest_name_prefix}0${count.index + 1}"
  ...
  # provisioner "local-exec" {
  #   command = "ssh -p ${var.guest_ssh_password} ssh-copy-id -i ${var.guest_ssh_key_public} -o StrictHostKeyChecking=no ${var.guest_ssh_user}@${self.guest_ip_addresses[0]}"
  # }
